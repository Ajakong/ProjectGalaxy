...
namespace MyEngine
{
	...
	private:
		...
		{
			std::shared_ptr<CollideData> own;
			std::shared_ptr<CollideData> send;
			OnCollideInfoKind kind;
		};
		using SendCollideInfo = std::unordered_map<std::shared_ptr<CollideData>, std::list<std::shared_ptr<CollideData>>>;
	private:
		Physics();
