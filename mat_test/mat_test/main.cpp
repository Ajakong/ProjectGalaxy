...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_  HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd)
{
	...
	int modelHandle = MV1LoadModel("Model/Cannon.mv1");
	...
	int cylinderIndex = MV1SearchFrame(modelHandle, "Cylinder");
	int standIndex = MV1SearchFrame(modelHandle, "Cube.002");

	MATRIX cylinderIdent = MV1GetFrameLocalMatrix(modelHandle, cylinderIndex);
	MATRIX standIdent = MV1GetFrameLocalMatrix(modelHandle, standIndex);

	MV1SetFrameUserLocalMatrix(modelHandle, cylinderIndex, cylinderIdent);
	MV1SetFrameUserLocalMatrix(modelHandle, standIndex, standIdent);
	...
	while (ProcessMessage() != -1)
	{
		...
		{
			Vec3 inputVec(directX, 0, directY);
			inputVec.Normalize();
			Vec3 neckPos = MV1GetFramePosition(modelHandle, cylinderIndex);
			standIdent = MV1GetFrameLocalMatrix(modelHandle, cylinderIndex);


			DrawLine3D(neckPos.VGet(), (neckPos + inputVec * 30).VGet(), 0xff0000);
			if (GetAngle(front, inputVec) <= DX_PI_F / 2)
			{
				MATRIX DirMat = MGetRotVec2(nowNeckDir.VGet(), inputVec.VGet());
				nowNeckDir = inputVec;
				MATRIX localMat = MV1GetFrameLocalMatrix(modelHandle, cylinderIndex);
				MATRIX mat = MMult(DirMat, localMat);
				MV1SetFrameUserLocalMatrix(modelHandle, cylinderIndex, mat);
			}
